...
*/
  ...
  public String uncode(String code, BSTNode<T> root) {
	  BSTNode<T> curr = root;
	  String message = "";
	  String english = "";
	  
	  for (int i = 0; i < code.length(); i++) {
		  // if theres a dot, go left, then add the letters of that path
		  if (code.charAt(i) == '.') {
			curr = curr.getLeft();
			message += curr.getInfo();
		  }
		  // if theres a space, signals end of word, so set pointer back to top
		  // then only take the last letter returned as the letter found
		  else if  (code.charAt(i) == ' ') {
				curr = root;
				// this takes the last letter
				// Example, if string is EISH, take only H and add to message variable
				// As in finding H in hello
				//System.out.print(message.substring(message.length() - 1));
				english += message.substring(message.length() - 1);
			}
		  // if theres dash, go right then add those letters to string
		  else if (code.charAt(i) == '-') {
			  curr = curr.getRight();
			  message += curr.getInfo();
		  }
		  // if thers a '#' symbol, add a space
		  else if (code.charAt(i) == '#') {
			  curr = root;
			  // similar to space, take last letter
			  //System.out.print(message.substring(message.length() - 1) + " ");
			  english += message.substring(message.length() - 1) + " ";
		  }
	  }
	  // gets the last letter of the inputted line from console
	  //System.out.print(message.substring(message.length() - 1));
	  english += message.substring(message.length() - 1);
